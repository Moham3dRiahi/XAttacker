...
make test
make install
cpan install Getopt::Long && cpan install HTTP::Request && cpan install LWP::UserAgent && cpan install IO::Select && cpan install HTTP::Cookies && cpan install HTTP::Response && cpan install Term::ANSIColor && cpan install URI::URL && cpan install IO::Socket::INET && cpan install Try::Tin
